...
class Scenario:
    ...
    def updateVehicles(self, observation, frame):
        ...
        ego_vehicle = observation.ego_vehicle_state
        neighbor_vehicle_tuple = observation.neighborhood_vehicle_states

        vehicle_obs_list = [ego_vehicle] + list(neighbor_vehicle_tuple)
        presence_vehicle_obs_list = []
        for i, vehicle_obs in enumerate(vehicle_obs_list):
            vid = 'veh' + str(i) if i !=0 else 'ego'
            presence_vehicle_obs_list.append(vid)
            x, y, _ = vehicle_obs.position
            speed = vehicle_obs.speed
            heading = vehicle_obs.heading

            # heading과 speed를 이용해 vx, vy 계산
            vx = speed * math.cos(heading)
            vy = speed * math.sin(heading)
            lane_id = 'lane' + vehicle_obs.lane_id[2::]

            vehicle = self.vehicles.get(vid, Vehicle(id=vid))
        ...
        for vid in self.vehicles.keys():
            if vid not in presence_vehicle_obs_list:
                self.vehicles[vid].clear()
